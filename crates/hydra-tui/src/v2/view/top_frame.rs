...
pub fn greeting_items(model: &str, genome_count: usize, mw_count: usize) -> Vec<crate::stream_types::StreamItem> {
    ...
    // Adaptive: use terminal width (subtract borders + divider)
    let term_width = crossterm::terminal::size().map(|(w, _)| w as usize).unwrap_or(80);
    let hw = (term_width.saturating_sub(3)) / 2; // half width for each panel
    let hw = hw.max(30).min(60); // reasonable bounds
    let sep = "─".repeat(hw);
    let left = [
