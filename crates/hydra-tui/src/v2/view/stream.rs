...
pub fn render(frame: &mut Frame, area: ratatui::layout::Rect, state: &RenderState) {
    ...
    for item in &state.stream_items {
        match item {
            ...
            StreamItem::AssistantText { text, .. } => {
                ...
                // Render with theme's assistant_text color (fixed in theme.rs)
    ...
    if state.stream_scroll_offset > 0 {
        ...
        let indicator = Paragraph::new(Line::from(vec![
            Span::styled("  ▼ more below (PageDown)", Style::default().fg(Color::Rgb(60, 60, 70)).add_modifier(Modifier::DIM)),
        ]));
