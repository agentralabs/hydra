...
fn cmd_help(_args: &str, _ctx: &CommandContext) -> Vec<StreamItem> {
    vec![
        ...
        sys(""),
        sys("  Control"),
        sys("    /do <goal>     Execute multi-step task via conductor"),
        sys("    /code <desc>   Generate code via O9 Supreme Coder"),
        sys("    /run <cmd>     Execute shell command"),
        sys("    /spawn <task>  Spawn fleet agent for a task"),
        sys("    /browse <url>  Fetch and extract URL content"),
        sys("    /websearch <q> Search the web"),
        sys("    /swarm <goal>  Multi-agent web research"),
        sys("    /shell         Enter interactive shell mode"),
        sys(""),
        sys("  Info"),
        sys("    /status        System status"),
        sys("    /health        Full health check (live Lyapunov value)"),
        sys("    /genome        Genome stats (/genome domains for breakdown)"),
        sys("    /memory        Memory layer breakdown"),
        sys("    /fleet         Fleet agent status"),
        sys("    /self          Self-model + capabilities (/self story, /self thoughts)"),
        sys("    /costs         Settlement cost breakdown"),
        sys("    /audit         Recent audit receipts"),
        sys("    /obstacles     Antifragile resistance built"),
        sys(""),
        sys("  Desktop"),
        sys("    /open <app>    Launch application"),
        sys("    /switch <win>  Focus window"),
        sys("    /windows       List open windows"),
        sys("    /capture       Screen capture"),
        sys(""),
        sys("  Session"),
        sys("    /resume        Resume last conversation"),
        sys("    /sessions      List saved conversations"),
        sys("    /save, /copy, /export"),
        sys(""),
        sys("  System"),
        sys("    /backup        Create/list/restore backups"),
        sys("    /voice         Voice setup and test"),
        sys("    /remote        Remote access URL/PIN"),
        sys("    /immerse       Domain mastery control"),
        sys("    /monitors      Active monitors and alerts"),
        sys("    /credentials   Manage login vault"),
        sys(""),
        sys("  Keyboard"),
        sys("    Ctrl+K         Command palette (fuzzy search all commands)"),
        sys("    Shift+Enter    Multi-line input"),
        sys("    Ctrl+V         Voice input (push-to-talk)"),
