...
body_template     = '{"symbol": "{symbol}", "qty": "{quantity}", "side": "buy"}'
```

## Read vs Write Capabilities

<CardGroup cols={2}>
  <Card title="Read Capabilities" icon="eye">
    No approval needed. Hydra checks automatically. Examples: portfolio value, build status, service health.
  </Card>
  <Card title="Write Capabilities" icon="pen">
...
| `body_template` | Request body with `{parameter}` placeholders | JSON template |
